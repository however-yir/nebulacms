...
class AuthorizationTest {
    ...
    void anonymousUserAccessProtectedApi() {
        ...
        webClient.get().uri("/apis/fake.nebulacms.io/v1/posts")
    ...
    void anonymousUserAccessAuthenticationFreeApi() {
        ...
        PolicyRule policyRule = new PolicyRule.Builder()
            .apiGroups("fake.nebulacms.io")
            .verbs("list")
            .resources("posts")
            .build();
        role.getRules().add(policyRule);
        when(roleService.listDependenciesFlux(anySet())).thenReturn(Flux.just(role));
        webClient.get().uri("/apis/fake.nebulacms.io/v1/posts").exchange().expectStatus()
            .isOk()
            .expectBody(String.class).isEqualTo("returned posts");

        webClient.get().uri("/apis/fake.nebulacms.io/v1/posts/hello-halo")
            .header("X-Requested-With", "XMLHttpRequest")
            .exchange()
            .expectStatus()
            .isUnauthorized();
    ...
    void anonymousUserShouldAccessResourcesByAggregatedRoles() {
        ...
        webClient.get().uri("/apis/fake.nebulacms.io/v1/fakes").exchange()
            .expectStatus()
            .isOk();
    }
    ...
    static class TestConfig {
        ...
        public RouterFunction<ServerResponse> postRoute() {
            return RouterFunctions.route()
                .GET("/apis/fake.nebulacms.io/v1/posts", request -> ServerResponse.ok()
                    .contentType(MediaType.TEXT_PLAIN)
                    .bodyValue("returned posts")
                )
                .PUT("/apis/fake.nebulacms.io/v1/posts/{name}", request -> ServerResponse.ok()
                    .contentType(MediaType.TEXT_PLAIN)
                    .bodyValue("updated post " + request.pathVariable("name"))
                )
                .GET("/apis/fake.nebulacms.io/v1/fakes", request -> ServerResponse.ok().build())
                .build();
        }
