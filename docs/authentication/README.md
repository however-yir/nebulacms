...
Halo 专门提供了生成 PAT 的端口：`/apis/api.console.security.nebulacms.io/v1alpha1/users/-/personalaccesstokens`。创建 PAT
请求示例如下：

```shell
curl -u admin:admin -X 'POST' \
  'http://localhost:8090/apis/api.console.security.nebulacms.io/v1alpha1/users/-/personalaccesstokens' \
  -H 'accept: application/json' \
...
| 属性路径                                                | 描述                                           |
|-----------------------------------------------------|----------------------------------------------|
| security.nebulacms.io/access-token | 生成好的 PAT。需要注意的是，这个 PAT 不会保存在数据库中，所以仅有一次保存机会。 |
...
```shell
curl http://localhost:8090/apis/api.console.nebulacms.io/v1alpha1/users/- \
