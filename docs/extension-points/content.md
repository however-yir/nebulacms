...
在插件中通过实现 `io.nebulacms.app.theme.ReactivePostContentHandler` 接口来实现文章内容扩展。
...
apiVersion: plugin.nebulacms.io/v1alpha1
...
在插件中通过实现 `io.nebulacms.app.theme.ReactiveSinglePageContentHandler` 接口来实现内容扩展。
