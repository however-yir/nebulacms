...
```yaml
apiVersion: notification.nebulacms.io/v1alpha1
kind: ReasonType
metadata:
  name: comment
spec:
...
订阅退订链接 API
规则：`/apis/api.notification.nebulacms.io/v1alpha1/subscriptions/{name}/unsubscribe?token={unsubscribeToken}`。
...
1. 获取个人中心获取用户通知列表的 APIs 规则：
   `GET /apis/api.notification.nebulacms.io/v1alpha1/userspaces/{username}/notifications`
2. 将通知标记为已读：`PUT /apis/api.notification.nebulacms.io/v1alpha1/userspaces/{username}/notifications/mark-as-read`
3.

批量将通知标记为已读：`PUT /apis/api.notification.nebulacms.io/v1alpha1/userspaces/{username}/notifications/mark-specified-as-read`
...
1. 获取通知器发送方配置：`GET /apis/api.console.nebulacms.io/v1alpha1/notifiers/{name}/sender-config`
2. 保存通知器发送方配置：`POST /apis/api.console.nebulacms.io/v1alpha1/notifiers/{name}/sender-config`

个人中心用户获取和保存对应通知器接收消息配置的 APIs:

1. 获取通知器接收消息配置：`GET /apis/api.notification.nebulacms.io/v1alpha1/notifiers/{name}/receiver-config`
2. 获取通知器接收消息配置：`POST /apis/api.notification.nebulacms.io/v1alpha1/notifiers/{name}/receiver-config`
