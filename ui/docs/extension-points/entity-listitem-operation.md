...
```ts
import type { ListedPost } from "@nebula-labs/api-client";
      ...
      return [
        {
          ...
          action: async (post: ListedPost) => {
            const { data } = await axios.get(
              `/apis/api.console.nebulacms.io/v1alpha1/posts/${post.post.metadata.name}/head-content`
            );
...
export interface OperationItem<T> {
  ...
  component: Raw<Component>;            // 菜单项组件，可以使用 `@nebula-labs/components` 中提供的 `VDropdownItem`，也可以自定义
