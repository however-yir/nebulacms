...
统一由 `@nebula-labs/ui-shared` 包中的 `definePlugin` 方法配置。如：
...
import { IconFolder } from "@nebula-labs/components";
...
interface RouteMeta {
  ...
  menu?: {
    ...
    icon?: Component;           // 菜单图标，类型为 Vue 组件，可以使用 `@nebula-labs/components` 包中的图标组件，或者自行接入 https://github.com/antfu/unplugin-icons
