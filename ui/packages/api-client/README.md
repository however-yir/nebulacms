...
```shell
pnpm install @nebula-labs/api-client axios
```
...
此外，在最新的 `@nebula-labs/ui-plugin-bundler-kit@2.17.0` 中，已经排除了 `@nebula-labs/api-client`、`axios` 依赖，所以最终产物中的相关依赖会自动使用 Halo 本身提供的依赖，无需关心最终产物大小。

详细文档可查阅：[插件开发 / API 请求](https://docs.nebulacms.io/developer-guide/plugin/api-reference/ui/api-request)
