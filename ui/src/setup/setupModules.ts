...
import type { PluginModule, RouteRecordAppend } from "@nebula-labs/ui-shared";
...
async function loadPluginBundle() {
  const { load } = useScriptTag(
    `/apis/api.console.nebulacms.io/v1alpha1/plugins/-/bundle.js?t=${Date.now()}`
  );
  await load();
}
...
async function loadPluginStyles() {
  await loadStyle(
    `/apis/api.console.nebulacms.io/v1alpha1/plugins/-/bundle.css?t=${Date.now()}`
  );
}
